...
First, in \texttt{Rstudio} create a new \texttt{R\ Notebook} file. A
default template will be opened. There is some \texttt{R} code in
\texttt{R\ chunks}, run them then click on \texttt{Preview} and save
your file. Then your html report will be opened in the viewer pane.
Every time you click on \texttt{Preview} the html file in the viewer
pane will be updated. If your file is named \texttt{report.Rmd}, your
report is named \texttt{report.nb.html}.

\begin{rmdcaution}
\begin{itemize}
\tightlist
\item
  If there is no \texttt{R\ Notebook} button, this means you don't have
  the last version of \texttt{Rstudio}.
\item
  In this case, click on \texttt{R\ Markdown} buttonn make sure that you
  select HTML as output. This produces a \texttt{html\_document} and you
  need to click on \texttt{knit} button to compile it.
\item
...
\item
  If it doesn't work and in order to not loose more time, write your
...
\begin{itemize}
\item
  Be ready to submit your report (your \texttt{.nb.html} file) at the
  end of each class.
\item
  You report must be named:
\end{itemize}

\texttt{YouLastName\_YourFirstName\_WeekNumber.nb.html}
\end{rmdcaution}
...
In \texttt{Rstudio}, start by creating a R Notebook. When you create it
a default template will be opened with the following first lines:
...
\item
  Run the chunk, you get the result down the chunk, don't close it.
\item
  Click on Preview.
\item
