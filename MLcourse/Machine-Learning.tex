...
\CommentTok{#ans>  [1]  1.1582 -0.5217  0.8044 -1.4379 -0.7891  0.0433 -1.4176 -0.1814}
\CommentTok{#ans>  [9]  0.4807 -0.5889}
...
For this week, download the following template by clicking on this
\href{files/LastName_FirstName_Week3.Rmd}{link} (right cligk and save
link as). Save the template in your folder that you are going to use as
working directory. Rename the file (replace LastName by your last name
and FirstName by your first name). Open it in \texttt{Rstudio} and use
it for your report to be submitted.
...
\textbf{3.} Is the target variable \texttt{Purchased} is of type factor?
If not, Encode it as a factor with levels 0 and 1.

\textbf{4.} Now we are going to split the dataset into training set and
...
\textbf{5.} Scale the input variables in both training set and test set.
...
\textbf{6.} In the argument \texttt{family} of the function \texttt{glm}
we chose \texttt{binomial}. Why ?

\textbf{7.} What is the equation of the obtained model in the question
1.5 ?

\textbf{8.} Is the feature \texttt{Age} significant?
...
\textbf{9.} Plot \texttt{Purchased} in function of \texttt{Age} and add
...
\textbf{10.} Now let us take another feature into account in the model.
Fit a logistic regression model of purchasing the product in function of
the age of the user and its salary.

\textbf{11.} Are the predictors significant? Did the model get better by
adding t he estimated salary?

\textbf{12.} On the test set, predict the probability of purchasing the
product by the users using the obtained model.

\textbf{13.} Take a look on your predicted values for the variable
...
\textbf{14.} Now to evaluate the predictions, compute the confusion
matrix. What do you obtain ?

(\textbf{Hint}: you can use the \texttt{table()} function).

\textbf{15.} Calculate the accuracy, specificity, sensitivity and the
precision of the model.

\textbf{16.} Plot the ROC curve and calculate AUC value.

\begin{rmdinsight}
\textbf{Hints}: to plot it, install the \texttt{RCOC} package. Load and
use the functions:
...
\textbf{17.} Compare the AUC of the two models you fitted (one with only
