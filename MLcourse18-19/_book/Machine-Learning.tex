...
\emph{sensitivity} (recall, or true positive rate) vs \emph{1-
specificity} (false positive rate). The area under the curve is referred
to as the \textbf{\emph{AUC}}, and is a numeric metric used to represent
the quality and performance of the classifier (model).
...
\item
  An example where \(\mu_1=-1.5\), \(\mu_2=1.5\), \(\pi_1=\pi_2=0.5\)
...
\begin{itemize}
\item
  See this \textcolor{white}{[}\faVideoCamera\textcolor{white}{]} video
  to understand more about \emph{decision boundary} (Applied on logistic
  regression).
\item
...
\CommentTok{# There is a categorical variable in the dataset, which is Gender.}
\CommentTok{# Of course we cannot show a boxplot of Gender and Purchased.}
\CommentTok{# But we can show a table, or a mosaic plot, both tell the same thing.}
\KeywordTok{table}\NormalTok{(dataset}\OperatorTok{$}\NormalTok{Gender,dataset}\OperatorTok{$}\NormalTok{Purchased)}
\CommentTok{# Remark for the function table(), that}
\CommentTok{# in lines we have the first argument, and in columns we have the second argument.}
\CommentTok{# Don't forget this when you use table() to show a confusion matrix!}
\KeywordTok{mosaicplot}\NormalTok{(}\OperatorTok{~}\StringTok{ }\NormalTok{Purchased }\OperatorTok{+}\StringTok{ }\NormalTok{Gender, }\DataTypeTok{data=}\NormalTok{dataset,}\DataTypeTok{main =} \StringTok{"MosaicPlot of two categorical variables: Puchased & Gender"}\NormalTok{,}\DataTypeTok{color =} \DecValTok{2}\OperatorTok{:}\DecValTok{3}\NormalTok{, }\DataTypeTok{las =} \DecValTok{1}\NormalTok{)}

\CommentTok{# since these 2 variables are categorical, we can apply}
\CommentTok{# a Chi-square test. The null hypothesis is the independance between}
\CommentTok{# these variables. You will notice that p-value = 0.4562 which is higher than 0.5}
\CommentTok{# so we cannot reject the null hypothesis. }
\CommentTok{# conclusion: there is no dependance between Gender and Purchased (who}
\CommentTok{# said that women buy more than men? hah!)}

\KeywordTok{chisq.test}\NormalTok{(dataset}\OperatorTok{$}\NormalTok{Purchased, dataset}\OperatorTok{$}\NormalTok{Gender)}

\CommentTok{# Let's say we want to remove the first two columns as we are not going to use them.}
\CommentTok{# But, we can in fact use a categorical variable as a predictor in logistic regression.}
\CommentTok{# It will treat it the same way as in regression. Check Appendix C.}
\CommentTok{# Try it by yourself if you would like to.}
\NormalTok{dataset =}\StringTok{ }\NormalTok{dataset[}\DecValTok{3}\OperatorTok{:}\DecValTok{5}\NormalTok{]}
\KeywordTok{str}\NormalTok{(dataset) }\CommentTok{# show the new structure of dataset}


\CommentTok{# splitting the dataset into training and testing sets}
\KeywordTok{library}\NormalTok{(caTools)}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{) }\CommentTok{# CHANGE THE VALUE OF SEED. PUT YOUR STUDENT'S NUMBER INSTEAD OF 123.}
\NormalTok{split =}\StringTok{ }\KeywordTok{sample.split}\NormalTok{(dataset}\OperatorTok{$}\NormalTok{Purchased, }\DataTypeTok{SplitRatio =} \FloatTok{0.75}\NormalTok{)}
\NormalTok{training_set =}\StringTok{ }\KeywordTok{subset}\NormalTok{(dataset, split }\OperatorTok{==}\StringTok{ }\OtherTok{TRUE}\NormalTok{)}
\NormalTok{test_set =}\StringTok{ }\KeywordTok{subset}\NormalTok{(dataset, split }\OperatorTok{==}\StringTok{ }\OtherTok{FALSE}\NormalTok{)}
...
\KeywordTok{plot}\NormalTok{(test_set[, }\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{],}
...
\KeywordTok{points}\NormalTok{(test_set[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{], }\DataTypeTok{pch =} \DecValTok{21}\NormalTok{, }\DataTypeTok{bg =} \KeywordTok{ifelse}\NormalTok{(test_set[, }\DecValTok{3}\NormalTok{] }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{, }\StringTok{'green4'}\NormalTok{, }\StringTok{'red3'}\NormalTok{))}
...
\textbf{Remark}: You are going to prove it in the practical work
session.
\end{rmdinsight}
...
is called a \textbf{\emph{biplot}}. An example is given later in this
chapter.
...
\begin{itemize}
\item
  You can read this \textcolor{white}{[}\faFilePdfO\textcolor{white}{]}
  tutorial . In the document, there is an introduction about the
  mathemtical concepts used in PCA. Plus a detailed example of PCA.
\item
...
The dataset
\textcolor{white}{[}\faTable\textcolor{white}{]}\texttt{eurojob}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-120-1} \end{center}
...
\CommentTok{#ans>  $ loadings: 'loadings' num [1:9, 1:9] 0.52379 0.00132 -0.3475 -0.25572 -0.32518 ...}
...
\CommentTok{# Scatterplot matrix of the scores - See how they are uncorrelated!}
\KeywordTok{scatterplotMatrix}\NormalTok{(pca}\OperatorTok{$}\NormalTok{scores, }\DataTypeTok{regLine =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{method=}\NormalTok{lm, }\DataTypeTok{lty=}\DecValTok{1}\NormalTok{, }\DataTypeTok{lwd=}\DecValTok{2}\NormalTok{, }\DataTypeTok{col=}\StringTok{"green"}\NormalTok{), }\DataTypeTok{smooth =} \OtherTok{FALSE}\NormalTok{,}
                  \DataTypeTok{ellipse =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{plot.points =}\NormalTok{ T, }\DataTypeTok{col=}\StringTok{"black"}\NormalTok{,}
                  \DataTypeTok{diagonal =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-121-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-123-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-123-2} \end{center}

\begin{Shaded}
\begin{Highlighting}[]

\CommentTok{# PC1 and PC2 -- These are the weights of the variables on the first two Principal Components}
...
\begin{center}\includegraphics[width=0.9\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-125-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# loading factoextra library}
\KeywordTok{require}\NormalTok{(factoextra)}
\CommentTok{# Calculating pca using prcomp(), which is a built-in function in R }
\NormalTok{res.pca <-}\StringTok{ }\KeywordTok{prcomp}\NormalTok{(eurojob, }\DataTypeTok{scale =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# Graph of individuals}
\KeywordTok{fviz_pca_ind}\NormalTok{(res.pca,}
             \DataTypeTok{col.ind =} \StringTok{"contrib"}\NormalTok{, }\CommentTok{# Color by their contribution to axes}
...
\begin{center}\includegraphics[width=0.9\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-126-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-126-2} \end{center}
...
\KeywordTok{fviz_pca_biplot}\NormalTok{(res.pca, }\DataTypeTok{repel =} \OtherTok{TRUE}\NormalTok{,}
                \DataTypeTok{col.var =} \StringTok{"#2E9FDF"}\NormalTok{, }
                \DataTypeTok{col.ind =} \StringTok{"#696969"}  
\NormalTok{                )}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.9\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-126-3} \end{center}
...
\addcontentsline{toc}{subsection}{The Iris Dataset}
...
\begin{center}\includegraphics[width=0.85\linewidth]{img/iris} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-132-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-134-1} \end{center}

\subsection*{\texorpdfstring{PCA using
\texttt{princomp()}}{PCA using princomp()}}\label{pca-using-princomp}
\addcontentsline{toc}{subsection}{PCA using \texttt{princomp()}}

```

\begin{rmdtip}
\texttt{princomp()} and \texttt{prcomp()} are built-in \texttt{R}
functions. They both perform PCA.
\end{rmdtip}

\textbf{4.} Apply a PCA on the Iris dataset using the
\texttt{princomp()}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pcairis=}\KeywordTok{princomp}\NormalTok{(iris[,}\OperatorTok{-}\DecValTok{5}\NormalTok{], }\DataTypeTok{cor=}\NormalTok{T) }
\CommentTok{# Note that we take only the numerical columns to apply PCA.}
\CommentTok{# now pcairis is a R object of type princomp}

\CommentTok{# To display the internal structure of pcairis}
\KeywordTok{str}\NormalTok{(pcairis)}
\CommentTok{#ans> List of 7}
\CommentTok{#ans>  $ sdev    : Named num [1:4] 1.706 0.96 0.384 0.144}
\CommentTok{#ans>   ..- attr(*, "names")= chr [1:4] "Comp.1" "Comp.2" "Comp.3" "Comp.4"}
\CommentTok{#ans>  $ loadings: 'loadings' num [1:4, 1:4] 0.522 -0.263 0.581 0.566 0.372 ...}
\CommentTok{#ans>   ..- attr(*, "dimnames")=List of 2}
\CommentTok{#ans>   .. ..$ : chr [1:4] "sepal_length" "sepal_width" "petal_length" "petal_width"}
\CommentTok{#ans>   .. ..$ : chr [1:4] "Comp.1" "Comp.2" "Comp.3" "Comp.4"}
\CommentTok{#ans>  $ center  : Named num [1:4] 5.84 3.05 3.76 1.2}
\CommentTok{#ans>   ..- attr(*, "names")= chr [1:4] "sepal_length" "sepal_width" "petal_length" "petal_width"}
\CommentTok{#ans>  $ scale   : Named num [1:4] 0.825 0.432 1.759 0.761}
\CommentTok{#ans>   ..- attr(*, "names")= chr [1:4] "sepal_length" "sepal_width" "petal_length" "petal_width"}
\CommentTok{#ans>  $ n.obs   : int 150}
\CommentTok{#ans>  $ scores  : num [1:150, 1:4] -2.26 -2.09 -2.37 -2.3 -2.39 ...}
\CommentTok{#ans>   ..- attr(*, "dimnames")=List of 2}
\CommentTok{#ans>   .. ..$ : NULL}
\CommentTok{#ans>   .. ..$ : chr [1:4] "Comp.1" "Comp.2" "Comp.3" "Comp.4"}
\CommentTok{#ans>  $ call    : language princomp(x = iris[, -5], cor = T)}
\CommentTok{#ans>  - attr(*, "class")= chr "princomp"}

\CommentTok{# To see the variance explained by the the pcs}
\KeywordTok{summary}\NormalTok{(pcairis) }
\CommentTok{#ans> Importance of components:}
\CommentTok{#ans>                        Comp.1 Comp.2 Comp.3  Comp.4}
\CommentTok{#ans> Standard deviation      1.706  0.960 0.3839 0.14355}
\CommentTok{#ans> Proportion of Variance  0.728  0.230 0.0368 0.00515}
\CommentTok{#ans> Cumulative Proportion   0.728  0.958 0.9948 1.00000}

\CommentTok{# To plot the variance explained by each pc}
\KeywordTok{plot}\NormalTok{(pcairis) }
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-138-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-138-2} \end{center}
...
\addcontentsline{toc}{subsection}{Deeper PCA using \texttt{factoextra}
package}
...
\addcontentsline{toc}{subsection}{Step-by-step PCA}
...
{\centering \includegraphics[width=0.65\linewidth]{img/clustering_comparison} 

}

\caption{Performance comparison of different clustering methods on different datasets}\label{fig:clusteringcomparison}
\end{figure}

In this chapter we will see the basics of the \textbf{partition
methods}, and one of the most well-known clustering techniques, namely
\textbf{\emph{\(k\)-means clustering}}.
...
{\centering \includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-165-1} 
...
{\centering \includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-168-1} 
...
In this practical work we will learn how to create a report in
\texttt{Rstudio} using \texttt{Rmarkdown} files. Then we will apply the
\(k\)-means clustering algorithm using the standard function
\texttt{kmeans()} in \texttt{R}, we will use the dataset
\texttt{Ligue1\ 2017/2018}.

\section*{Reporting}\label{reporting-1}
\addcontentsline{toc}{section}{Reporting}
...
\begin{rmdcaution}
\textbf{Very Important Remark}: Click on the settings button of
Rstudio's text editor and choose to Chunk Output in Console.

\begin{figure}
\centering
\includegraphics{img/chunk_output_in_console.png}
\caption{}
\end{figure}
\end{rmdcaution}
...
\addcontentsline{toc}{subsection}{\texttt{Ligue\ 1}}
...
\addcontentsline{toc}{subsection}{\texttt{Implementing\ k-means}}
...
EM}\label{gaussian-mixture-models-em}

In the previous chapter we saw the \(k\)-means algorithm which is
considered as a hard clustering technique, such that each point is
allocated to only one cluster. In \(k\)-means, a cluster is described
only by its centroid. This is not too flexible, as we may have problems
with clusters that are \emph{overlapping}, or ones that are not of
\emph{circular shape}.

In this chapter, we will introduce a model-based clustering technique,
which is \textbf{E}xpectation \textbf{M}aximization (EM). We will apply
it using Gaussian Mixture Models (GMM).

With EM Clustering, we can go a step further and describe each cluster
by its centroid (mean), covariance (so that we can have elliptical
clusters), and weight (the size of the cluster). The probability that a
point belongs to a cluster is now given by a multivariate Gaussian
probability distribution (multivariate - depending on multiple
variables). That also means that we can calculate the probability of a
point being under a Gaussian `bell', i.e.~the probability of a point
belonging to a cluster. A comparison between the performances of
\(k\)-means and EM clustering on an artificial dataset is shown in
Figure \ref{fig:ClusterAnalysisMouse}.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/ClusterAnalysis_Mouse} 

}

\caption{Comparison of $k$-means and EM on artificial data called [Mouse](https://elki-project.github.io/datasets/) dataset. Using the Variances, the EM algorithm can describe the normal distributions exact, while $k$-means splits the data in [Voronoi](https://bit.ly/1rVyJkt)-Cells.}\label{fig:ClusterAnalysisMouse}
\end{figure}
...
properties, it suffers from significant limitations when it comes to
modeling real data sets. Consider the example shown in Figure
\ref{fig:gaussianfaithful} applied on the 'Old Faithful' data set, this
data set comprises 272 measurements of the eruption of the
\href{https://en.wikipedia.org/wiki/Old_Faithful}{Old Faithful geyser}
at Yellowstone National Park in the USA. Each measurement comprises the
duration of the eruption in minutes (horizontal axis) and the time in
minutes to the next eruption (vertical axis). We see that the data set
forms two dominant clumps, and that a simple Gaussian distribution is
unable to capture this structure, whereas a linear superposition of two
Gaussians gives a better characterization of the data set. Such
superpositions, formed by taking linear combinations of more basic
distributions such as Gaussians, can be formulated as probabilistic
models known as \emph{mixture distributions}.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/gaussian_faithful} 

}

\caption{Plots of the ’old faithful’ data in which the blue curves show contours of constant probability density. On the left is a single Gaussian ditribution which has been fitted to the data using maximum likelihood. On the right the distribution is given by a linear combination of two Gaussians which has been fitted to the data by maximum likelihood using the EM technique, and which gives a better representation of the data}\label{fig:gaussianfaithful}
\end{figure}

In Figure \ref{fig:gaussianmixture} we see that a linear combination of
Gaussians can give rise to very complex densities. By using a sufficient
number of Gaussians, and by adjusting their means and covariances as
well as the coefficients in the linear combination, almost any
continuous density can be approximated to arbitrary accuracy.

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{img/gaussian_mixture} 

}

\caption{Example of a Gaussian mixture distribution in one dimension showing three Gaussians (each scaled by a coefficient) in blue and their sum in red.}\label{fig:gaussianmixture}
\end{figure}

We therefore consider a superposition of \(K\) Gaussian densities of the
form

\[\label{eq:gaussian}
p(x)= \sum_{k=1}^{K} \pi_k \mathcal{N}(x| \mu_k, \Sigma_k)\]

which is called a \emph{mixture of Gaussians}. Each Gaussian density
\(\mathcal{N}(x| \mu_k, \Sigma_k)\) is called a \emph{component} of the
mixture and has its own mean \(\mu_k\) and covariance \(\Sigma_k\).

The parameters \(\pi_k\) are called \emph{mixing coefficients}. They
verify the conditions
\[\sum_{k=1}^{K} \pi_k = 1 \quad \text{and} \quad 0 \leq \pi_k \leq 1\]

In order to find an equivalent formulation of the Gaussian mixture
involving an explicit \textbf{latent} variable, we introduce a
\(K\)-dimensional binary random variable \(z\) having a 1-of-\(K\)
representation in which a particular element \(z_k\) is equal to 1 and
all other elements are equal to 0. The values of \(z_k\) therefore
satisfy \(z_k \in \{0,1\}\) and \(\sum_k z_k =1\), and we see that there
are \(K\) possible states for the vector \(z\) according to which
element is nonzero. The marginal distribution over \(z\) is specified in
terms of the mixing coefficients \(\pi_k\) , such that
\[p(z_k=1)=\pi_k\]

\begin{rmdinsight}
A \href{https://en.wikipedia.org/wiki/Latent_variable}{latent variable}
is a variable that is not directly measurable, but its value can be
inferred by taking other measurements.

This happens a lot in machine learning, robotics, statistics and other
fields. For example, you may not be able to directly quantify
...
{\centering \includegraphics[width=0.9\linewidth]{img/gaussianmixture_500samples} 

}

\caption{Example of 500 points drawn from a mixture of 3 Gaussians. (a) Samples from the joint distribution $p(z)p(x|z)$ in which the three states of $z$, corresponding to the three components of the mixture, are depicted in red, green, and blue, and (b) the corresponding samples from the marginal distribution $p(x)$, which is obtained by simply ignoring the values of $z$ and just plotting the $x$ values. The data set in (a) is said to be complete, whereas that in (b) is incomplete. (c) The same samples in which the colours represent the value of the responsibilities $r(z_{nk})$ associated with data point $x_n$, obtained by plotting the corresponding point using proportions of red, blue, and green ink given by $r(z_{nk})$ for $k = 1,2,3$, respectively.}\label{fig:gaussianmixture500samples}
\end{figure}

So the form of the Gaussian mixture distribution is governed by the
parameters \(\pi\), \(\mu\) and \(\Sigma\), where we have used the
notation \(\pi=\{\pi_1,\ldots,\pi_K\}\), \(\mu=\{\mu_1,\ldots,\mu_K\}\)
and \(\Sigma=\{\Sigma_1,\ldots,\Sigma_K\}\). One way to set the values
of these parameters is to use maximum likelihood. The log of the
...
a single Gaussian, due to the presence of the summation over \(k\)
inside the logarithm. As a result, the maximum likelihood solution for
the parameters no longer has a closed-form analytical solution. One
approach to maximizing the likelihood function is to use iterative
numerical optimization techniques. Alternatively we can employ a
powerful framework called \textbf{E}xpectation \textbf{M}aximization,
...
\item
  ...
  \[\ln p(X|\pi,\mu,\Sigma)=\sum_{n=1}^{N} \ln \left\lbrace \sum_{k=1}^{K} \pi_k \mathcal{N}(x_n | \mu_k, \Sigma_k) \right\rbrace\]
  Until convergence of either the parameters or the log likelihood. If
  the convergence criterion is not satisfied return to E step.
\end{itemize}\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

The EM algorithm for a mixture of two Gaussians applied to the rescaled
Old Faithful data set is illustrated in Figure \ref{fig:emfaithful}. In
...
{\centering \includegraphics[width=0.8\linewidth]{img/em_faithful} 

}

\caption{Illustration of the EM algorithm using the Old Faithful dataset. A mixture of two Gaussians is used.}\label{fig:emfaithful}
\end{figure}
...
For this session, write your report in a RMarkdown script, in which you
must use the following YAML header settings (replace the default YAML
header by this one, edit the author's name and show the date):
...
\section{\texorpdfstring{EM using
\texttt{mclust}}{EM using mclust}}\label{em-using-mclust}

\subsection*{\texorpdfstring{GMM vs
\(k\)-means}{GMM vs k-means}}\label{gmm-vs-k-means}
\addcontentsline{toc}{subsection}{GMM vs \(k\)-means}

In this section, we will use two artificial (simulated) datasets in
which we know the ground truth (true labels) in order to compare the
performances of \(k\)-means and GMM. To fit a GMM using EM technique you
need to install and use the package \texttt{mclust}.

\textbf{1.} Download and import
\textcolor{white}{[}\faTable\textcolor{white}{]} Data1 and
\textcolor{white}{[}\faTable\textcolor{white}{]} Data2 . Plot both of
the datasets on the same window. Color the observations with respect to
the ground truth, like in Figure \ref{fig:fig1}.

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/fig1-1} 

}

\caption{Data1 is plotted on left. Data2 on right. Colors shown with respect to ground truth.}\label{fig:fig1}
\end{figure}
...
parameterisations of the within-group covariance matrix \(\Sigma_k\). In
GMM, clusters are ellipsoidal, centered at the mean vector \(\mu_k\),
and with other geometric features, such as volume, shape and
orientation, determined by the covariance matrix \(\Sigma_k\).

\begin{figure}
\centering
\includegraphics{img/gmm_mclust_sigma.png}
\caption{}
\end{figure}
\end{rmdinsight}

\textbf{7.} Though GMM is often categorized as a clustering algorithm,
fundamentally it is an algorithm for density estimation. That is to say,
the result of a GMM fit to some data is technically not a clustering
model, but a generative probabilistic model describing the distribution
of the data. Density estimation plays an important role in applied
statistical data analysis and theoretical research. A density estimate
based on GMM can be obtained using the function
\texttt{densityMclust()}. Apply it on Data2 and visualize the estimated
densities (show an ``image'' and a ``perspective'' plot of the bivariate
density estimate).

\subsection*{EM on 1D}\label{em-on-1d}
\addcontentsline{toc}{subsection}{EM on 1D}

In this part you must fit a GMM model on a one dimensional simulated
data.
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-214-1} \end{center}
...
{\centering \includegraphics[width=0.6\linewidth]{img/dendro} 

}

\caption{Simulated data of 45 observations generated from a three-class model.}\label{fig:dendro}
\end{figure}

The data in the figure above consists of 45 observations in
two-dimensional space. The data were generated from a three-class model;
the true class labels for each observation are shown in distinct colors.
However, suppose that the data were observed without the class labels,
and that we wanted to perform hierarchical clustering of the data.
Hierarchical clustering (with complete linkage, to be discussed later)
yields the result shown in Figure \ref{fig:dendro2}. How can we
interpret this dendrogram?

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{img/dendro2} 

}

\caption{Dendrogram}\label{fig:dendro2}
\end{figure}

In the dendrogram of Figure \ref{fig:dendro2}, each leaf of the
dendrogram represents one of the 45 observations. However, as we move up
the tree, some leaves begin to fuse into branches. These correspond to
observations that are similar to each other. As we move higher up the
tree, branches themselves fuse, either with leaves or other branches.
The earlier (lower in the tree) fusions occur, the more similar the
groups of observations are to each other. On the other hand,
observations that fuse later (near the top of the tree) can be quite
different. In fact, this statement can be made precise: for any two
observations, we can look for the point in the tree where branches
containing those two observations are first fused. The height of this
fusion, as measured on the vertical axis, indicates how different the
two observations are. Thus, observations that fuse at the very bottom of
the tree are quite similar to each other, whereas observations that fuse
close to the top of the tree will tend to be quite different.

An example of interpreting a dendrogram is presented in Figure
\ref{fig:dendro3}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{img/dendro3} 

}

\caption{An illustration of how to properly interpret a dendrogram with nine observations in two-dimensional space. Left: a dendrogram generated using Euclidean distance and complete linkage. Observations 5 and 7 are quite similar to each other, as are observations 1 and 6. However, observation 9 is no more similar to observation 2 than it is to observations 8, 5, and 7, even though observations 9 and 2 are close together in terms of horizontal distance. This is because observations 2, 8, 5, and 7 all fuse with observation 9 at the same height, approximately 1.8. Right: the raw data used to generate the dendrogram can be used to confirm that indeed, observation 9 is no more similar to observation 2 than it is to observations 8, 5, and 7.}\label{fig:dendro3}
\end{figure}

Now that we understand how to interpret the dendrogram of Figure
\ref{fig:dendro2}, we can move on to the issue of identifying clusters
on the basis of a dendrogram. In order to do this, we make a horizontal
cut across the dendrogram, as shown in the following Figure
\ref{fig:dendro4} where we cut the dendro4 at a height of nine results
in two clusters.

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{img/dendro4} 

}

\caption{The dendrogram from the simulated dataset, cut at a height of nine (indicated by the dashed line). This cut results in two distinct clusters, shown in different colors.}\label{fig:dendro4}
\end{figure}

The distinct sets of observations beneath the cut can be interpreted as
clusters. In Figure \ref{fig:dendro5}, cutting the dendrogram at a
height of five results in three clusters.

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{img/dendro5} 

}

\caption{The dendrogram from the simulated dataset, cut at a height of five (indicated by the dashed line). This cut results in three distinct clusters, shown in different colors.}\label{fig:dendro5}
\end{figure}

The term \emph{hierarchical} refers to the fact that clusters obtained
by cutting the dendrogram at a given height are necessarily nested
within the clusters obtained by cutting the dendrogram at any greater
height.

\section{The Hierarchical Clustering
Algorithm}\label{the-hierarchical-clustering-algorithm}

The hierarchical clustering dendrogram is obtained via an extremely
simple algorithm. We begin by defining some sort of \emph{dissimilarity}
measure between each pair of observations. Most often, Euclidean
distance is used. The algorithm proceeds iteratively. Starting out at
the bottom of the dendrogram, each of the n observations is treated as
its own cluster. The two clusters that are most similar to each other
are then fused so that there now are \(n−1\) clusters. Next the two
clusters that are most similar to each other are fused again, so that
there now are \(n − 2\) clusters. The algorithm proceeds in this fashion
until all of the observations belong to one single cluster, and the
dendrogram is complete.

Figure \ref{fig:cah} depicts the first few steps of the algorithm.

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{img/cah} 

}

\caption{An illustration of the first few steps of the hierarchical clustering algorithm, with complete linkage and Euclidean distance. Top Left: initially, there are nine distinct clusters {1}, {2}, ..., {9}. Top Right: the two clusters that are closest together, {5} and {7}, are fused into a single cluster. Bottom Left: the two clusters that are closest together, {6} and {1},are fused into a single cluster. Bottom Right: the two clusters that are closest together using complete linkage, {8} and the cluster {5, 7}, are fused into a single cluster.}\label{fig:cah}
\end{figure}

To summarize, the hierarchical clustering algorithm is given in the
following Algorithm:
...
\begin{itemize}
\item
  \textbf{Complete}: Maximal intercluster dissimilarity. Compute all
  ...
  dissimilarities.
\item
  \textbf{Single}: Minimal intercluster dissimilarity. Compute all
  pairwise dissimilarities between the observations in cluster A and the
...
\item
  \textbf{Average}: Mean intercluster dissimilarity. Compute all
  pairwise dissimilarities between the observations in cluster A and the
  observations in cluster B, and record the average of these
  dissimilarities.
\item
  \textbf{Centroid}: Dissimilarity between the centroid for cluster A (a
  mean vector of length p) and the centroid for cluster B. Centroid
  linkage can result in undesirable inversions.
\end{itemize}

Average and complete linkage are generally preferred over single
linkage, as they tend to yield more balanced dendrograms. Centroid
linkage is often used in genomics.

The dissimilarities computed in Step 2(b) of the hierarchical clustering
algorithm will depend on the type of linkage used, as well as on the
choice of dissimilarity measure. Hence, the resulting dendrogram
typically depends quite strongly on the type of linkage used, as is
shown in Figure \ref{fig:linkage}.

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{img/linkage} 

}

\caption{Average, complete, and single linkage applied to an example data set. Average and complete linkage tend to yield more balanced clusters.}\label{fig:linkage}
\end{figure}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-219-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-219-2} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-219-3} \end{center}
...
\addcontentsline{toc}{chapter}{PW 9}

\section*{\texorpdfstring{Distances
\texttt{dist()}}{Distances dist()}}\label{distances-dist}
\addcontentsline{toc}{section}{Distances \texttt{dist()}}
...
\addcontentsline{toc}{section}{Dendrogram \texttt{hclust()}}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-224-1} \end{center}

\section*{Hierarchical clustering on Iris
dataset}\label{hierarchical-clustering-on-iris-dataset}
\addcontentsline{toc}{section}{Hierarchical clustering on Iris dataset}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(dendro.avg, }\DataTypeTok{hang=}\OperatorTok{-}\DecValTok{1}\NormalTok{, }\DataTypeTok{label=}\NormalTok{sampleiris}\OperatorTok{$}\NormalTok{class)}
\end{Highlighting}
\end{Shaded}
...
\chapter{References and Credits}\label{references-and-credits}

Here are the references I used the most for this course:

\begin{itemize}
\tightlist
\item
  The Elements of Statistical Learning for Trevor Hastie, Robert
  Tibshirani and Jerome H. Friedman, link .
\item
  Pattern Recognition and Machine Learning for Christopher Bishop, link
  .
\item
  Andrew Ng's Machine Learning Mooc, link .
\item
  Lab notes for Statistics for Social Sciences II: Multivariate
  Techniques, link .
\end{itemize}
...
\tightlist
\item
  This website was written in Rmarkdown and compiled using
  \href{https://bookdown.org/}{bookdown}.
\end{itemize}

\chapter{Other References}\label{other-references}
