...
\CommentTok{#ans>  [1] -0.6563 -1.0385 -0.0699  0.9699  1.7918 -1.5374  0.0420 -0.8714}
\CommentTok{#ans>  [9]  0.3257  0.4683}
...
\midrule
\endhead
Factor (Explained) & \(p-1\) & TSS & MST=SST/\((p-1)\) & F=MST/MSE &
p-value\tabularnewline
Error (Unexplained) & \(n-p\) & RSS & MSE=SSE/\((n-p)\) &
&\tabularnewline
...
\subsection{Use the Adjusted R2 for univariate
models}\label{use-the-adjusted-r2-for-univariate-models}

If you only use one input variable, the \texttt{adjusted\ \$R\^{}2\$}
value gives you a good indication of how well your model performs. It
illustrates how much variation is explained by your model.

In contrast to the simple \texttt{\$R\^{}2\$}, the adjusted
\texttt{adjusted\ \$R\^{}2\$} takes the number of input factors into
account. It penalizes too many input factors and favors parsimonious
models.

The adjusted R2 is sensitive to the amount of noise in the data. As
such, only compare this indicator of models for the same dataset than
comparing it across different datasets.
