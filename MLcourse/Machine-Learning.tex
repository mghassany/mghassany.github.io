...
\textbf{3.} Split the iris dataset into data \(X\) and class labels
\(y\).
...
\textbf{4.} To explore how the 3 different flower classes are
distributed along the 4 different features, visualize them via
histograms.

\textbf{5.} Compare the means and the quartiles of the 3 different
flower classes for the 4 different features (Plot 4 boxplots into the
same figure).

\textbf{Standardizing}

\textbf{6.} Scale the 4 different features.
...
features (Do it manually without using the \texttt{cov()} function)
(Print the results).
...
question (use conditional statements to make the comparison).
...
\textbf{11.} Sort the eigenvalues from highest to lowest in order to
choose the top \(k\) eigenvectors.

\textbf{Explained Variance}

\textbf{12.} Calculate the individual explained variance and the
cumulative explained variance of each principal component. Show the
results.

\textbf{13.} Visualize the individual and cumulative explained variance
on the same figure.
...
respect to their flower classes and label them with respect of their id
(row number).
...
\textbf{18.} Use the \texttt{princomp()} function as explained in
Chapter 6. Compare the obtained results using \texttt{princomp()} with
your results above. Use conditional statement in order to compare the
results.

\textbf{19.} Plot together the scores for PC1 and PC2 and the variables
expressed in terms of PC1 and PC2 using \texttt{biplot()}.

\textbf{20.} Interpret the results.
