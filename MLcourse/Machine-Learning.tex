...
\item
  Set \(x_{2}=0\) and \(y_{2}=-1\). Recompute the previous expressions.
  (Answers: \texttt{97}, \texttt{2.785875})
\item
  Index \(y\) by \(x+1\) and store it as \texttt{z}. What is the output?
  (Answer: \texttt{z} is \texttt{c(-1,\ 100,\ 97,\ 96)})
\end{itemize}
\end{rmdexercise}
...
\texttt{Tools\ /\ Set\ working\ directory}.
...
\tightlist
\item
  If the file is a csv format, we use \texttt{read.csv}.
\item
  ...
  \texttt{;} or ..
\item
  I suggest \href{https://www.sublimetext.com/}{\texttt{Sublime\ Text}}
  or \href{https://atom.io/}{\texttt{Atom}} as text editors.
\end{itemize}
\end{rmdinsight}
...
\NormalTok{Auto[}\DecValTok{1}\NormalTok{:}\DecValTok{4}\NormalTok{,]}
...
short introduction to R} can be useful.
\end{rmdinsight}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# lm has the syntax: lm(formula = response ~ predictor, data = data)}
...
\CommentTok{#ans> Error in eval(predvars, data, env): invalid 'envir' argument of type 'character'}
...
\NormalTok{mod}
\CommentTok{#ans> Error in eval(expr, envir, enclos): object 'mod' not found}
...
\CommentTok{#ans> Error in summary(mod): object 'mod' not found}
\NormalTok{sumMod}
\CommentTok{#ans> Error in eval(expr, envir, enclos): object 'sumMod' not found}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \texttt{.RData} file and load it into use the function \texttt{load}.
\item
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-29-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-30-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-36-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{Machine-Learning_files/figure-latex/unnamed-chunk-37-1} \end{center}
